...
@section('content')
    <div id="catelogues">
        <div class="breadcrumb__nk">
            <div class="container"><a href="{{ route('home') }}" class="text-light">{{ trans('language.home') }} </a> / {{ trans('language.manual') }}</div>
        </div>
        <div class="container text-center">
            <h1 class="page_title">{{ trans('language.manual') }}</h1>
        </div>
					...
					<div class="tab-content mt-4" id="myTabContent">
						@foreach($brands as $brand)
							<div class="tab-pane fade @if($loop->iteration == 1) show active @endif" id="{{ $brand->id }}" role="tabpanel" aria-labelledby="{{ $brand->id }}-tab">
								<div class="row">
									@php
										$catalogues = Cache::remember("catalogues{$brand->id}", now()->addHours(1), function () use ($brand) {
											return \App\Models\Catalogue::where('type','manual')->where('brand_id', $brand->id)->where('status', 1)->get();
										});
									@endphp
