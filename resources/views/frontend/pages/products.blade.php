                                ...
                                <form id="desktopFilterForm">
                                    <div class="accordion-item">
                                        <h2 class="accordion-header">
                                            <button class="accordion-button {{ $root_category ? '' : 'collapsed' }}"
                                        ...
                                        <div id="desktopCategories"
                                            class="accordion-collapse collapse {{ $root_category ? 'show' : '' }}">
                                    ...
                                    <div class="accordion-item">
                                        <h2 class="accordion-header">
                                            <button class="accordion-button {{ $current_brand ? '' : 'collapsed' }}"
                                                type="button" data-bs-toggle="collapse" data-bs-target="#desktopBrands">
                                                <i class="fa-solid fa-tag" style="color:var(--pr);"></i>
                                                {{ trans('language.brands') }}
                                            </button>
                                        </h2>

                                        <div id="desktopBrands"
                                            class="accordion-collapse collapse {{ $current_brand ? 'show' : '' }}">
                                            <div class="accordion-body">
                                                @foreach ($brands as $brand)
                                                    <div class="form-check filter-check">
                                                        <input class="form-check-input brands_for_filter" type="checkbox"
                                                            value="{{ $brand->id }}" id="dbrand_{{ $brand->id }}"
                                                            data-label="{{ $brand->title }}" @checked(optional($current_brand)->id == $brand->id)>
                                        ...
                                        <div class="accordion-item">
                                            <h2 class="accordion-header">
                                                <button class="accordion-button collapsed" type="button"
                                                    data-bs-toggle="collapse" data-bs-target="#{{ $desktopAttrId }}">
                                            ...
                                            <div id="{{ $desktopAttrId }}" class="accordion-collapse collapse">
                                                <div class="accordion-body">
                                                    @foreach ($attributes->attributes_values as $value)
                                                        @php
                                                            $desktopValId =
                                                                'dattr_' . $loop->parent->index . '_' . $loop->index;
                                                        @endphp

                                                        <div class="form-check filter-check">
                                                            <input class="form-check-input attributes_for_filter"
                                                                type="checkbox" value="{{ $value->value }}"
                                                                id="{{ $desktopValId }}" data-label="{{ $value->value }}">
                        ...
                        <div class="search-bar-wrap">
                            <div id="liveSearchWrapper">
                                <div class="search-input-group">
                                    ...
                                    <input type="text" id="productNameInput" name="name" autocomplete="off"
                                        spellcheck="false" placeholder="Search product, code or brand...">
                            ...
                            @foreach ($categories as $category)
                                <div class="form-check filter-check">
                                    <input class="form-check-input category_for_filter" type="checkbox"
                                        ...
                                        data-label="{{ $category->title }}" @checked(optional($root_category)->id == $category->id)>
                                ...
                                @foreach ($attributes->attributes_values as $value)
                                    ...
                                    <div class="form-check filter-check">
                                        <input class="form-check-input attributes_for_filter" type="checkbox"
                                            value="{{ $value->value }}" id="{{ $mobileValId }}"
    ...
    @push('footer')
        <script>
            (function($) {
                ...
                /*
                 * Back/forward restore: the browser's own scroll restoration is disabled
                 * so it cannot fight with ours while AJAX pages are still loading.
                 */
                ...
                let restoring = false;
                let restoreRunId = 0;
                let userScrolledSinceRestore = false;
                ...
                function updateFilterUI() {
                    ...
                    $('.category_for_filter:checked, .brands_for_filter:checked, .attributes_for_filter:checked').each(
                        function() {
                    ...
                    chips.forEach(function(chip) {
                        $wrap.append(
                            '<span class="filter-chip" data-filter-class="' + escapeHtml(chip.cls) +
                ...
                function updateListingMeta(response) {
                    ...
                    if (listState.total > 0) {
                        $('#listingCount').html(
                            'Showing <strong>' + shown + '</strong> of <strong>' + listState.total +
                            '</strong> products'
                        );
                    } else {
                ...
                function fetchPage(page, append, perPageOverride) {
                    ...
                    /* A fresh listing always goes back to the normal page size. */
                    const effectivePerPage = perPageOverride || listState.perPage || 20;
                    if (perPageOverride) {
                        listState.perPage = effectivePerPage;
                    }

                    const fd = buildFormData();
                    fd.set('page', page);
                    fd.set('per_page', effectivePerPage);
                ...
                if ('IntersectionObserver' in window && sentinel) {
                    ...
                    $(window).on('scroll.productsInfinite', function() {
                        const nearBottom = window.innerHeight + window.scrollY >= document.documentElement
                            .scrollHeight - 500;
                ...
                function renderSuggestions(items, query) {
                    ...
                    if (!suggestItems.length) {
                        $box.removeClass('d-none').html(
                            '<div class="ps-empty"><i class="fa-solid fa-box-open"></i><br>' + escapeHtml(NO_RESULT) +
                            '</div>'
                        );
                        return;
                    }
                ...
                $('#productNameInput').on('keydown', function(e) {
                    ...
                    if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
                        ...
                        e.preventDefault();
                        saveState(); // Save position before navigating to the product page.
                ...
                /*
                 * Keep scrollY up to date while the user scrolls, so the state stays
                 * correct even if pagehide/visibilitychange never fire (mobile).
                 */
                $(window).on('scroll.productsState', function() {
                    if (restoring) return;

                    clearTimeout(scrollSaveTimer);
                    scrollSaveTimer = setTimeout(saveState, 350);
                });

                /*
                 * Real user interactions stop the programmatic re-scroll retries,
                 * so the user always wins over the restore logic.
                 */
                $(window).on('wheel.productsState touchmove.productsState mousedown.productsState keydown.productsState',
                    function() {
                        if (restoring) {
                            userScrolledSinceRestore = true;
                        }
                    });
                ...
                function scrollToSavedY(targetY) {
                    if (typeof targetY !== 'number' || isNaN(targetY)) return;

                    window.scrollTo(0, targetY);

                    /*
                     * Re-apply a few times while late layout (lazy images etc.) settles,
                     * unless the user already started scrolling on their own.
                     */
                    [200, 600, 1400, 2500].forEach(function(delay) {
                        setTimeout(function() {
                            if (!userScrolledSinceRestore) {
                                window.scrollTo(0, targetY);
                            }
                        }, delay);
                    });
                ...
                function restoreState(saved) {
                    ...
                    restoring = true;
                    userScrolledSinceRestore = false;
                    ...
                    /*
                     * Fast restore: instead of re-fetching every page one by one,
                     * ask the server for all previously seen products in one request
                     * (per_page = targetPage * perPage, capped at 200 by the server).
                     */
                    const savedPerPage = Math.max(1, Number(saved.perPage || 20));
                    const targetPage = Math.max(1, Number(saved.page || 1));
                    const wantedCount = targetPage * savedPerPage;

                    const firstRequest = fetchPage(
                        1,
                        false,
                        targetPage > 1 ? Math.min(wantedCount, 200) : undefined
                    );
                    ...
                    function finishRestore() {
                        ...
                        const have = $('#productListing [role="listitem"]').length;

                        if (have < wantedCount && listState.page < listState.lastPage) {
                            /* Server capped the page size — pull the remaining products. */
                            fetchPage(listState.page + 1, true).then(finishRestore, finishRestore);
                            return;
                        }

                        restoring = false;
                        scrollToSavedY(Number(saved.scrollY));
                        saveState();
                    }

                    firstRequest.then(finishRestore, finishRestore);
                }
                ...
                $(window).on('pageshow', function(event) {
                    if (event.originalEvent && event.originalEvent.persisted) {
                        ...
                        restoring = false;
                        scrollToSavedY(saved ? Number(saved.scrollY) : window.scrollY);
                    }
                });
                ...
                /*
                 * Restore on:
                 *  - real back/forward loads (navigation type "back_forward"), or
                 *  - a navigation to the same listing URL within 60 seconds
                 *    (covers browsers/webviews that report back loads as "navigate").
                 */
                const savedState = readState();
                const isBackForward = navType === 'back_forward';
                const isRecentSameUrl = savedState &&
                    savedState.url === window.location.href &&
                    (Date.now() - Number(savedState.savedAt || 0)) < 60000;

                restoreState(
                    isBackForward || (navType === 'navigate' && isRecentSameUrl) ? savedState : null
                );
