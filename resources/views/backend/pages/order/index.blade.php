    ...
    @push('footer')
        <script type="text/javascript">
                ...
                if (go_next_step == true) {
                    ...
                    $.ajax({
                        ...
                        error: function(xhr) {
                            ...
                            $('#createOrderForm .server_side_error').html(
                ...
                if (go_next_step == true) {
                    ...
                    $.ajax({
                        ...
                        error: function(xhr) {
                            ...
                            $('#editOrderForm .server_side_error').html(
                                '<div class="alert alert-danger" role="alert">' + errorMessage +
                                '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>'
                                );
                        },
            ...
            $(document).on('click', '.view_btn', function(e) {
                ...
                $.ajax({
                    url: "{{ url('/admin/order/view/') }}/" + id,
                    type: "GET",
                    dataType: "html",
                    success: function(data) {
                        // Create modal dynamically if not exists
                        if ($('#viewOrderModal').length === 0) {
                            $('body').append(
                                '<div class="modal fade" id="viewOrderModal" tabindex="-1" data-bs-backdrop="static"></div>'
                                );
                        }
                        $('#viewOrderModal').html(
            ...
            $(document).on('click', '#statusOrderBtn', function(e) {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        ...
                        $('#statusOrderForm .server_side_error').html(
                            '<div class="alert alert-danger" role="alert">' + errorMessage +
                            '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>'
                            );
                    },
