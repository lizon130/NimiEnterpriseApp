...
@section('content')
    <div class="container-fluid px-4">
        ...
        <div class="card my-2">
            <form action="{{ route('admin.product.bulk.action') }}" method="POST" style="width: 100%;"
                id="bulk_option_form">
                <div class="card-header">
                    <div class="row ">
                        <div class="col-12 d-flex justify-content-between">
                            <div class="w-50 d-flex align-items-center justify-content-between">
                                <h5 class="w-50">Product List</h5>
                            ...
                            <div>
                                ...
                                @if (Helper::hasRight('custom-field.view'))
                                    <a href="{{ route('admin.product.custom.field') }}" class="btn btn-info text-light">
                                        Custom Fields</a>
                                @endif
                            </div>
                        </div>
        ...
        <script type="text/javascript">
            ...
            function getproduct(category = null, brand = null, name = null) {
                var table = jQuery('#dataTable').DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: "{{ url('admin/product/get/list') }}",
                        type: 'GET',
                        data: {
                            'category': category,
                            'brand': brand,
            ...
            $(document).on('click', '#createProductBtn', function(e) {
                ...
                if (go_next_step == true) {
                    ...
                    let meta_descriptions = $('#createProductForm #meta_descriptions').summernote('code');
                    ...
                    $.ajax({
                        ...
                        error: function(xhr) {
                            ...
                            $('#createProductForm .server_side_error').html(
            ...
            $(document).on('change', '#createModal .main_category', function(e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('#createModal .create_category_select2').html(data.category);
                        $('.create_category_select2').select2({
                            dropdownParent: $('#createModal')
                        });
                    }
            ...
            $(document).on('change', '#editModal .main_category', function(e) {
                e.preventDefault();
                let parent_id = $(this).val();
                $.ajax({
                    url: "{{ route('admin.product.get.subcategory') }}",
                    ...
                    dataType: "json",
                    success: function(data) {
                        $('#editModal .edit_category_select2').html(data.category);
                ...
                if (go_next_step == true) {
                    ...
                    $.ajax({
                        ...
                        error: function(xhr) {
                            ...
                            $('#editProductForm .server_side_error').html(
                                '<div class="alert alert-danger" role="alert">' + errorMessage +
                                '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>'
                            );
                        },
            ...
            $(document).on('click', '#createModal #addAttributes', function(e) {
                ...
                $('#createModal .attributes_area tbody').append(
                    ...
                    '<td class="text-center"><a href="" type="button" class="btn btn-sm btn-danger remove_attributes"  id="attributes' +
                    create_attributes_row + '"><i class="fa fa-trash"></i></a></td>' +
                    '</tr>'
                );
            })
            ...
            function getCustomOptionModel(id) {
                $.ajax({
                    url: "{{ url('/admin/product/custom-option/') }}/" + id,
                    type: "GET",
                    dataType: "html",
                    success: function(data) {
            ...
            $(document).on('change', '.custom_field_id', function(e) {
                ...
                if (id != '') {
                    $.ajax({
                        url: "{{ url('/admin/product/custom-option/sub-option/') }}/" + id,
                        type: "GET",
                        dataType: "html",
                        success: function(data) {
            ...
            function generateHtmlFroCustomOption(custom_field_id, sub_option, product_id) {
                $.ajax({
                    url: "{{ url('/admin/product/custom-option-generate-html') }}",
                    type: "GET",
                    data: {
            ...
            function deleteCustomOption(custom_field_id, sub_option, product_id) {
                $.ajax({
                    url: "{{ url('/admin/product/custom-option-delete') }}",
                    type: "GET",
                    data: {
                        'custom_field_id': custom_field_id,
                        'sub_option': sub_option,
            ...
            $(document).on('click', '#updateCustomOptionBtn', function(e) {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        ...
                        $('#customProductForm .server_side_error').html(
                            '<div class="alert alert-danger" role="alert">' + errorMessage +
                            '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>'
                        );
            ...
            function removeFile(element) {
                ...
                Swal.fire({
                    title: 'Are you sure?',
                    text: "You won't be able to revert this file again!",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
