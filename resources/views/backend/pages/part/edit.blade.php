                        ...
                        <div class="col-lg-6">
                            ...
                            <select name="category_id" class="form-control">
                                ...
                                @foreach ($category as $cat)
                                    <option @if($part->category_id == $cat->id) selected @endif value="{{ $cat->id}}">{{ $cat->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-lg-6">
                            <label for="">Parts Name<span class="text-danger">*</span></label>
                            <input type="text" name="name" class="form-control" value="{{ $part->getTranslation(Session::get('admin_language') ?? 'en', 'name') ?? '' }}" placeholder="Parts Name" {{ old('name')}} required>
                        </div>
                    </div>
							...
							@if(count($images) > 0)
								@foreach($images as $key => $val)
									<div class="position-relative ms-1">
										<img src="{{ asset('uploads/part-images/' . $val) }}" height="80px" width="100px" class="mt-1 border" alt="">
										<a href="{{ route('admin.remove.file', ['model' => 'ProductPart', 'id' => $part->id, 'file_name' => $val]) }}" onclick="removeFile(this);" class="position-absolute top-0 end-0 rounded-circle bg-danger text-light px-1"><i class="fa-solid fa-xmark"></i></a>
									</div>
								@endforeach
							@endif
						</div>
