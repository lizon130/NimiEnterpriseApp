...
class PartnerController extends Controller
{
    ...
    public function index(){
        ...
        return view('backend.pages.partner.index', compact('partners', 'category', 'products'));
    }
    ...
    public function update(Request $request, $id){
        ...
        if($request->hasFile('image')){
            if ($company->website_logo != null && file_exists(public_path('uploads/user-images/'.$company->website_logo))) {
                unlink(public_path('uploads/user-images/'.$company->website_logo));
            }
            $image = $request->file('image');
            $filename = time().uniqid().$image->getClientOriginalName();
            $image->move(public_path('uploads/user-images'), $filename);
    ...
    public function delete($id){
        $partner = Company::find($id);
        if($partner){
            if ($partner->website_logo != null && file_exists(public_path('uploads/user-images/'.$partner->website_logo))) {
                unlink(public_path('uploads/user-images/'.$partner->website_logo));
            }
            if ( $partner->delete()) {
