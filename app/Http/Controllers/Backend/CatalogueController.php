...
class CatalogueController extends Controller
{
    ...
    public function index(){
        $categorys = Category::where('is_parent', 1)->get();
        $brands = Brand::all();
    ...
    public function store(Request $request){
        ...
        if ($catalogue->save()) {
            Helper::insertLanguage(Catalogue::class, $catalogue->id, Session::get('admin_language') ?? 'en', 'file', $catalogue->file);
    ...
    public function localDelete($id){
        ...
        if($translation){
            if ($translation->file != null && file_exists(public_path('uploads/catalogue-images/'.$translation->file))) {
                unlink(public_path('uploads/catalogue-images/'.$translation->file));
            }
            $translation->delete();
            return redirect()->back()->with(['success' => 'Catalogue file deleted successfully.']);
        }else{
            return redirect()->back()->withErrors(['error' => 'Catalogue file not found.']);
        }
    }
}
