...
class ResourceController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $resource->short_number = $request->short_number;
        ...
        if ($resource->save()) {
			Resource::where('short_number', '>=', $request->short_number)
			->where('type','=',$request->type)
			->where('id', '!=', $resource->id)
			->increment('short_number');
    ...
    public function update(Request $request, $id){
        ...
        if ($resource->save()) {
			$oldShorterOrder = $resource->short_number;
			
			if(intval($request->short_number) > intval($oldShorterOrder)){
				Resource::where('short_number', '<=', $request->short_number)
				->where('type','=',$request->type)
				->where('id', '!=', $resource->id)
				->where('short_number', '>', $oldShorterOrder)
				->decrement('short_number');
			}else{
				Resource::where('short_number', '>=', $request->short_number)
				->where('type','=',$request->type)
				->where('id', '!=', $resource->id)
    ...
    public function delete($id){
        $resource = Resource::find($id);
        if($resource){
            if ($resource->image != null && file_exists(public_path('uploads/resource-images/'.$resource->image))) {
                unlink(public_path('uploads/resource-images/'.$resource->image));
            }
            $resource->delete();
